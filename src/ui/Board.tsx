...
interface Props { pieces: readonly Piece[]; perspective: Player; selected: Site | null; targets?: readonly Site[]; onSelect(site: Site): void }
export function Board({ pieces, perspective, selected, targets = [], onSelect }: Props) {
  const rows = Array.from({ length: 8 }, (_, i) => perspective === 1 ? 8 - i : i + 1);
  return <div className="board" aria-label="開発用盤面（全駒表示）">
    ...
    {rows.map((row, index) => <div className={`board-row ${index === 3 ? 'border-row' : ''}`} key={row}>
      {SITES.filter(s => anchors(s)[0].y === row).sort((a, b) => (anchors(a)[0].x - anchors(b)[0].x) * (perspective === 1 ? 1 : -1)).map(site => {
        ...
        const label = piece ? `P${piece.owner} ${PIECES[piece.type].label}` : '空き';
        return <button key={site} type="button" data-site={site} aria-label={`${site} ${label}`} aria-pressed={selected === site}
          ...
          <span className="piece-label">{piece ? PIECES[piece.type].label : targets.includes(site) ? '●' : isHQ(site) ? '司令部' : '·'}</span>

...
開発サーバーはlocalhost限定です。トップ画面の「コンピューターと対戦」から難易度を選び、自軍23枚の配置を交換・確定すると対局を開始します。駒と金枠の移動先を選び、着手内容を確認して実行します。直前の着手は盤面と文章で確認でき、全履歴は折りたためます。「続きから」は有効な自動保存がある場合だけ使えます。既存の進行中対局を置き換える前には確認を表示します。開発用ローカル対局はトップ画面の「開発用・将来の機能」内に分離しています。
